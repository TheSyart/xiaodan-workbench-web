...
function AssistantDrawer({open,onClose}:{open:boolean;onClose:()=>void}){
  const[text,setText]=useState('');const[messages,setMessages]=useState<Array<{role:'user'|'assistant';text:string}>>([{role:'assistant',text:'我是小单。可以帮你查询项目、任务、账目、日历和稿件元数据。稿件正文只在编辑器里由你采纳。'}]);
  const send=async()=>{const prompt=text.trim();if(!prompt)return;setText('');setMessages((old)=>[...old,{role:'user',text:prompt}]);let tool='projects.list';if(prompt.includes('任务'))tool='tasks.list';else if(prompt.includes('账'))tool='finance.list';else if(prompt.includes('稿'))tool='scripts.search';try{const data=await post<unknown>('/assistant/execute',{tool,arguments:{query:prompt}});setMessages((old)=>[...old,{role:'assistant',text:`已查询：${JSON.stringify(data,null,2).slice(0,1200)}`}]);}catch(error){setMessages((old)=>[...old,{role:'assistant',text:error instanceof Error?error.message:'执行失败'}]);}};
  return <div className={`assistant-sheet ${open?'open':''}`} aria-hidden={!open}><header><div><span className="brand-mark small">小</span><div><strong>小单</strong><small>全局工作助理</small></div></div><button onClick={onClose}>关闭</button></header><div className="chat-feed">{messages.map((message,index)=><div key={index} className={`chat-message ${message.role}`}>{message.text}</div>)}</div><form onSubmit={(event)=>{event.preventDefault();void send();}}><textarea value={text} onChange={(event)=>setText(event.target.value)} placeholder="例如：看看我有哪些进行中的项目"/><button className="primary">发送</button></form></div>;
}
